//! CLI command handlers — status, index, reindex, reset.
//!
...
fn load_config(repo: &Path) -> anyhow::Result<(Config, std::path::PathBuf)> {
...
pub fn run_status(repo: &Path) -> anyhow::Result<()> {
    ...
    // Model availability — uses configured model paths
...
pub fn run_index(repo: &Path) -> anyhow::Result<()> {
    let (config, db_path) = load_config(repo)?;
    let cogz_dir = repo.join(".cogz");
    let storage = cogz::storage::Storage::open(&db_path)?;
