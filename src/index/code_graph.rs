...
pub fn sync_code_edges(
    ...
    for (rel_path, source, language) in source_files {
        ...
        for ce in entities {
            ...
            if let Some(simple) = qualified_name.rsplit("::").next() {
                name_to_uuid.entry(simple.to_string()).or_insert_with(|| id.clone());
            }
        }
    }
    ...
    for (rel_path, source, language) in source_files {
        ...
        match language {
            ...
            Language::Python => extract_python_edges(
                &root,
                source_bytes,
                &file_path_str,
                &name_to_uuid,
...
fn extract_rust_calls_in_node(
    ...
    walk_descendants(node, &mut cursor, &mut f);
}
...
fn extract_python_import(
    ...
    // For `import X` or `from X import Y`, try to match module/file names.
    let mut cursor = node.walk();
    for child in node.named_children(&mut cursor) {
        let name = node_text(&child, source).unwrap_or_default();
        // Try full path and last segment.
...
fn extract_python_calls_in_node(
    ...
    // Get the function's UUID.
    let func_name = node
        .child_by_field_name("name")
        .and_then(|n| node_text(&n, source))
        .unwrap_or_default();

    // For methods, we need the class context. We'll use the simple
    // name lookup — the qualified name was registered during sync.
    let source_id = name_to_uuid
        .get(&func_name)
        .cloned()
        .or_else(|| {
            // Try as file-level function
...
fn walk_descendants<F>(node: &Node, cursor: &mut TreeCursor, f: &mut F)
where
    F: FnMut(&Node) -> bool,
{
    // Process the current node's named children recursively.
    ...
    for i in 0..num_named {
        if let Some(child) = node.named_child(i) {
            ...
            walk_descendants(&child, cursor, f);
        }
    }
}
...
"#;
        ...
        let calls_count: i64 = conn
            ...
            .unwrap();
        assert!(calls_count >= 1, "expected at least 1 calls edge, got {calls_count}");
    }
