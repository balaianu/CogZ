...
my-project/
  .cogz/
    ...
    observations/        — empty, ready for observation files
    .gitignore           — generated: ignores cogz.db (disposable, rebuildable)
```
...
Knowledge, rules, and observations are NOT gitignored — they're meant
to be version-tracked as shared institutional memory.
...
- **`cogz.db`** — gitignored, local only. Sharing it via Git would
  ...
  invariant.
- **Observations** — git-tracked by default. Observations are engineering
  discoveries (bugs, gotchas, design insights) that form shared institutional
  memory. Solo developers who want local-only observations can use
  `cogz init --local-only` to gitignore all of `.cogz/`.
